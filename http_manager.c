...
#include <assert.h>
#include <pthread.h>
#include <unistd.h>  // 添加这个头文件，用于 sleep 函数
 ...
 * @brief 天气显示回调函数（内部使用）
 */
static void weather_display_callback(char* weather_info)
{
    printf("\n========== 天气信息 ==========\n");
    printf("%s\n", weather_info);
    printf("==============================\n");
...
/**
 * @brief 封装好的天气显示函数
 * 在主程序中直接调用此函数即可显示天气信息
 */
void tianqixianshi()
{
    const char *api_key = "SnZx2oM4I9NNXP1vi";  // 请替换为您的实际API密钥
    const char *city = "dalian";                // 可以修改为您需要的城市
    
    static int initialized = 0;  // 静态标志，确保只初始化一次
    
    // 如果还没初始化，先初始化HTTP管理器
    if (!initialized) {
        if (http_request_create() != 0) {
            printf("天气系统初始化失败！\n");
            return;
        }
